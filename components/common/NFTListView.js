...
import { bulkGetNftViews, getNftMetadataViews } from "../../flow/scripts";
import {
...
export default function NFTListView(props) {
  ...
  const checkNeedRelink = (collection, display) => {
    ...
    return (
      display.thumbnail && display.thumbnail.url && display.thumbnail.url == ""
    );
  };
  ...
  const getImageSrc = (file) => {
    ...
    if (src == "/token_placeholder.png") {
      return collection.squareImage
        ? getImageSrcFromMetadataViewsFile(collection.squareImage.file)
    ...
    if (displayData) {
      ...
      setDisplays((oldState) => {
        ...
        if (
          oldArray.length == 0 ||
          (newArray.length > 0 &&
            oldArray[oldArray.length - 1].tokenID !=
              newArray[newArray.length - 1].tokenID)
        ) {
  ...
  return (
    <div className="px-1 overflow-auto w-full">
      <div className="inline-block min-w-full">{showDisplays()}</div>
    </div>
  );
}
