...
export default function CollectionView(props) {
  ...
  return (
    <div
      className="flex flex-col max-w-[1094px] min-w-[1076px] gap-y-3 p-4 shadow-md rounded-2xl bg-white"
      onClick={() => {
      ...
      <div className="flex gap-x-3 justify-between">
        <div className="flex items-center gap-x-3">
          <div className="w-11 rounded-full overflow-hidden aspect-square relative">
            <Image
              src={
                collection.squareImage
                  ? getImageSrcFromMetadataViewsFile(
                      collection.squareImage.file
                    )
          ...
          <div className="flex flex-col w-full">
            ...
            <div className={`truncate text-ellipsis overflow-hidden shrink`}>
              {`/storage/`}
              <span className="font-bold">{`${collection.path.replace(
                "/storage/",
                ""
              )}`}</span>
            </div>
        ...
        <div className="flex gap-x-2 items-center">
          {collection.collectionIdentifier ? (
            <label className={`cursor-pointer shrink-0`}>
              <div
                className="h-[28px] aspect-square shrink-0 relative"
                onClick={(event) => {
                  window.open(
                    `${publicConfig.nftCatalogURL}/${collection.collectionIdentifier}`
                  );
