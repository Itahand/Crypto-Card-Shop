...
export default function Collection(props) {
  ...
  const [currentStoredItems, setCurrentStoredItems] = useRecoilState(
    currentStoredItemsState
  );
    ...
    if (account && isValidFlowAddress(account)) {
      if (
        ...
        (currentStoredItems.length > 0 &&
          currentStoredItems[0].address != account)
      ) {
        bulkGetStoredItems(account)
          .then((items) => {
  ...
  useEffect(() => {
    if (collectionData) {
      const newCollection = collectionsWithExtraData(
  ...
  useEffect(() => {
    if (
      ...
      !collections[0].addedCatalogInfo
    ) {
    ...
    if (collections) {
      if (hideEmptyCollections) {
        setFilteredCollections(
          collections.filter((c) => c.tokenIDs.length > 0)
        );
      } else {
        ...
        <div className="flex w-full flex-col gap-y-3 overflow-auto">
          ...
          <div className="px-2 py-2 overflow-x-auto max-h-screen w-full">
            <div className="inline-block min-w-full">
              <div className="flex flex-col gap-y-4">{showCollections()}</div>
            </div>
          </div>
        </div>
      </Layout>
